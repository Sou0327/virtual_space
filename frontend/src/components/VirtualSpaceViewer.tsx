...
import RoomBuilder from './three/RoomBuilder';
...
export const VirtualSpaceViewer: React.FC<VirtualSpaceViewerProps> = ({
  ...
  const getBackgroundGradient = (type: string) => {
    switch (type) {
      ...
      case 'custom': return 'from-white to-gray-100';
      ...
      <div className="absolute top-4 right-4 flex space-x-2">
        {isEntered && (
          <button
            ...
            title={is3DMode ? '2Dビューに戻る' : 'AIルームビルダーに切り替え'}
          >
            {is3DMode ? '2D' : 'AI'}
          </button>
        )}
